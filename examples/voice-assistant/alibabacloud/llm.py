        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, data=json.dumps(data), timeout=60000) as resp:
                ...
                async for block in resp.content.iter_any():
                    logging.info(f"llm received block: {block}")
                    lines = block.decode('utf-8').splitlines(keepends=False)
                    logging.info(f"llm received: {lines}")
                    for line in lines:
                        if line:
...
class LLMStream(llm.LLMStream):
    ...
    async def __anext__(self) -> llm.ChatChunk:
        answer = ""
        logging.info(f"llm self._oai_stream : {self._oai_stream}")
                ...
                if line.startswith("data:"):
                    ...
                    try:
                        ...
                        #     answer += delta["content"]
                         # 检查是否存在"output"键，并且"output"中是否有"text"   and chunk["output"]["finish_reason"] == "stop 加了这句话就不是流式处理了
                        if "output" in chunk and "text" in chunk["output"]:
                           # delta = chunk["output"]["text"]

                            answer += chunk["output"]["text"]
                                # 提取"text"对应的值
                            logging.info(f"llm return : {answer}")
                            ...
                            logging.info(f"llm return 空")
                    except json.JSONDecodeError as err:
                        logging.info(f"llm JSONDecodeError error : {err}")
